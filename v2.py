...
# We always want to start with a dataset to train on. Let's download the tiny shakespeare dataset
...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        # perform the weightefd aggregation of values
        v = self.value(x)
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        x = token_embeddings + pos_emb # (B, T, C)
        x = self.blocks(x)
        x = self.ln_f(x)
